...
async function processDocuments({ productProposal, userPersona, agentFramework }) {
  ...
  const getContent = (doc) => {
    if (doc.type === 'text' || doc.type === 'file') {
      return doc.content || 'Not provided';
  ...
  const productContent = getContent(productProposal);
  const personaContent = getContent(userPersona);
  const agentContent = getContent(agentFramework);
